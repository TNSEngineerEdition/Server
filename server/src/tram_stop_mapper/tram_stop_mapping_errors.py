...
class TramStopMappingErrors(BaseModel):
    ...
    underutilized_relations: dict[overpy.Relation, list[str | None]] = Field(
        default_factory=dict
    )

    @property
    def _missing_relations_for_lines_error_message(self):
    ...
    @property
    def _stops_without_mapping_error_message(self):
        return ", ".join(sorted(self.stops_without_mapping))

    @property
    def _underutilized_relations_error_message(self):
        return "\n\n".join(
